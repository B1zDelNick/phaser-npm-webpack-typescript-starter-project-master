...
export default class Select extends Phaser.State {
    ...
    private sel5: Phaser.Button = null;
    ...
    public create(): void {
        ...
        this.sel5 = GuiUtils.makeButton(
            this, this.container, 470, 363, 1,
            'sel5', ImageUtils.getAtlasClass('AtlasesStateSelect').getName(),
            GameConfig.CURRENT_STATE >= 4 ?
                ImageUtils.getAtlasClass('AtlasesStateSelect').Frames.Sel5 :
                ImageUtils.getAtlasClass('AtlasesStateSelect').Frames.Sel5G,
            GameConfig.CURRENT_STATE >= 4, true, true,
            this.nextState, null, null
        );
        ...
        TweenUtils.fadeAndScaleIn(this.sel5, Phaser.Timer.SECOND * .5, Phaser.Timer.SECOND * 3, () => {

            if (GameConfig.CURRENT_STATE === 0) this.sel1.filters = [EffectUtils.makeLightGlowAnimation(0xffff66, 1000, true, 99999, 250)];
            if (GameConfig.CURRENT_STATE === 0) EffectUtils.makeScaleAnimation(this.sel1);
            if (GameConfig.CURRENT_STATE === 1) this.sel2.filters = [EffectUtils.makeLightGlowAnimation(0xffff66, 1000, true, 99999, 250)];
            if (GameConfig.CURRENT_STATE === 1) EffectUtils.makeScaleAnimation(this.sel2);
            if (GameConfig.CURRENT_STATE === 2) this.sel3.filters = [EffectUtils.makeLightGlowAnimation(0xffff66, 1000, true, 99999, 250)];
            if (GameConfig.CURRENT_STATE === 2) EffectUtils.makeScaleAnimation(this.sel3);
            if (GameConfig.CURRENT_STATE === 3) this.sel4.filters = [EffectUtils.makeLightGlowAnimation(0xffff66, 1000, true, 99999, 250)];
            if (GameConfig.CURRENT_STATE === 3) EffectUtils.makeScaleAnimation(this.sel4);
            if (GameConfig.CURRENT_STATE === 4) this.sel5.filters = [EffectUtils.makeLightGlowAnimation(0xffff66, 1000, true, 99999, 250)];
            if (GameConfig.CURRENT_STATE === 4) EffectUtils.makeScaleAnimation(this.sel5);
        ...
        if (GameConfig.IS_ASSETS_LOADED)
            ...
            if (GameConfig.CURRENT_STATE === 3) PreloaderUtils.preloadDress4State();
            if (GameConfig.CURRENT_STATE === 4) PreloaderUtils.preloadPlaceState();
        ...
        if (GameConfig.PUB_MODE === PublishMode.NORMAL ||
            ...
            if (!GameConfig.AD1_SHOWED && GameConfig.CURRENT_STATE === 3) {
                GameConfig.AD1_SHOWED = true;
                AdUtils.playAds();
            }
        }
    ...
    private nextState(sprite: Phaser.Button): void {
        if (sprite === this.sel1) this.NEXT = 'Make';
        if (sprite === this.sel2) this.NEXT = 'Hair';
        if (sprite === this.sel3) this.NEXT = 'Dress';
        if (sprite === this.sel4) this.NEXT = 'Acsess';
        if (sprite === this.sel5) this.NEXT = 'Place';
        this.gui.disable();
        if (this.saver) {
